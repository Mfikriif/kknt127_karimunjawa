...
export default function EditSeaweedType({ seaweedType }: Props) {
    const [name, setName] = useState(seaweedType.name || '');
    const [characteristics, setCharacteristics] = useState(seaweedType.characteristics || '');
    const [benefits, setBenefits] = useState(seaweedType.benefits || '');
                ...
                <main className="flex-1 p-8 max-w-5xl mx-auto">
                    ...
                    >
                        <div className="grid grid-cols-2 gap-x-8 gap-y-6 items-start">
                            ...
                            {/* Gambar Lama */}
                            <div></div>
                            {seaweedType.image && !previewUrl && (
                                <div className="mt-6">
                                    <p className="text-sm text-gray-500 mb-2">Gambar saat ini:</p>
                                    <img
                                        src={`/storage/${seaweedType.image}`}
                                        alt={seaweedType.name}
