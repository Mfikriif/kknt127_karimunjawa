...
export default function Edit() {
  ...
  const [steps, setSteps] = useState(method.steps.map((step) => ({
    ...step,
    new_gambar: null as File | null,
    preview: step.gambar_tahapan ? `/storage/${step.gambar_tahapan}` : '',
  })));

  const handleStepChange = (index: number, field: 'deskripsi_tahapan' | 'new_gambar', value: any) => {
    const updated = [...steps];
                ...
                <div key={step.id} className="border border-gray-200 bg-gray-50 rounded-lg p-5 shadow-sm">
                  ...
                  <div className="mb-2">
                    ...
                    {step.preview && (
                      <img
                        src={step.preview}
                        alt={`Preview Tahap ${step.tahap_ke}`}
                        className="mb-3 rounded-md border w-48 h-auto object-cover"
                      />
                    )}
                    ...
                    <label className="inline-flex items-center gap-3 cursor-pointer bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm shadow-sm hover:bg-gray-100 transition">
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => handleStepChange(index, 'new_gambar', e.target.files?.[0] || null)}
                        className="hidden"
                      />
                    </label>
                  </div>
